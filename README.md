...
과목을 만들면 과목 상세에 4개 탭이 있다: **채팅 | 퀴즈 | 시험 | 노트**. 오답 목록·분석·다시 풀기는 퀴즈 화면 안에 있다.
...
| **채팅** | 과목 자료 전체를 신뢰하지 않는 사용자 입력 JSON으로 전달하는 튜터 채팅(출처 자료 제목 표시). 고정 안전 규칙과 허용 Skill만 system/developer 지침에 둔다. 입력창 위 토글로 **자료 기반 / 일반 질문** 모드 전환 — 일반 질문은 자료 없이 일반 지식으로 답변. 대화는 DB에 저장, 최근 30개가 컨텍스트로 전달됨 |
...
| **오답** | 퀴즈에서 틀린 문제 자동 축적(wrong_count>0) + 학교 시험지·문제집 사진으로 **오답 직접 등록**(from_wrong_note=1, wrong_count=1로 삽입). **약점 분석**(AI가 오답 패턴 요약, 저장 안 함), **오답만 다시 풀기**(퀴즈 탭으로 전환 후 즉시 출제) |
...
| **노트 (단권화)** | 선택 자료의 고유 시험 개념·공식·정의·함정·풀이 팁을 중복 없이 압축한 마크다운 노트. 출처 목록은 넣지 않는다. 서로 다른 과목은 동시에 단권화할 수 있고, 긴 단권화 하나는 AI 슬롯을 최대 2개만 쓴다. 짧은 블록·색상·배경·얇고 선명한 소단원 구분선과 공식별 네모 박스를 앱과 HTML 저장에 동일하게 적용한다. 표 안 절댓값 수식의 `|`가 열 구분자로 오인되는 문법도 저장·렌더 양쪽에서 자동 보정한다. 자연어 추가 요청, 직접 편집·저장, 버전 기록 지원 |
...
| POST /api/subjects/:id/chat ⚡ | `{message, mode?: "materials"\|"general"}` | `{reply}` |
...
| GET /api/subjects/:id/wrong | | wrong_count>0 문제, 많이 틀린 순 |
