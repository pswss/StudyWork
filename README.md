...
> 이 문서는 사용자용 안내이자, 이후 세션의 개발자(AI 포함)가 코드베이스를 정확히 이해하기 위한 기준 문서다. 마지막 갱신: 2026-08-08.
...
과목을 만들면 과목 상세에 6개 탭이 있다: **채팅 | 퀴즈 | 해설 | 시험 | 노트 | 설정**. 오답 목록·분석·다시 풀기는 퀴즈 탭 상단의 **문제 목록 / 오답 노트** 토글 안에 있다.
...
| **퀴즈** | 문제 목록: 문제집·프린트 사진/PDF에서 **문제 추출**, 선택 자료 기반 **AI 생성**(개수 1~20, 난이도 하/중/상/혼합), 또는 **2028 수능형 AI 모의고사**를 만든다. 모의고사는 국어 45·수학 30·영어 45·한국사 20·통합사회/통합과학/직업탐구 각 25·제2외국어/한문 20문항의 공식 기본 틀과 번호별 유형·배점·난이도·공유 지문 순서를 고정한다. 영어 듣기는 대본의 화자를 구분해 기기에 설치된 자연 음성으로 재생한다. 생성된 회차는 공식 번호순 풀이와 문제지/정답지 인쇄를 지원한다. 모든 AI 생성은 백그라운드에서 독립 검산 후 한 트랜잭션으로 저장한다. 일반 퀴즈는 자료 전체·자료별 전체·개별 문제 범위와 난이도·출처·개수·오답 필터를 교차해 출제한다. |
...
공통: 단일 소유자 계정(scrypt 비밀번호 해시, 버전이 있는 HMAC 세션 쿠키, 로그아웃 즉시 전체 세션 폐기), AI 호출 하루 200회 상한(`usage_daily` 카운터).

## 아키텍처

```
폰/PC/아이패드 브라우저 (React + Vite SPA, web/dist)
   │  로컬 개발 HTTP 또는 Tailscale Serve HTTPS
   ▼
내 맥의 Node 서버 — src/server.ts (Hono, HTTPS 모드 127.0.0.1:8787 / 로컬 개발 0.0.0.0:8787)
...
```bash
npm install
cp .env.example .env    # 10자 이상 bootstrap 비밀번호·32바이트 이상 서명 키를 .env에만 입력
...
API 키 대신 로컬 Codex CLI에 저장된 ChatGPT 로그인을 재사용한다. 공통·비채팅 작업의 모델과 effort는 **설정** 탭에서 저장하고, 채팅 전용 값은 **채팅** 탭에서 바로 조정한다. 기본값은 `gpt-5.6-sol` / `high`다.
...
```
src/
  server.ts       Node 엔트리: .env 로드, DB/FileStore 생성+마이그레이션, 정적 서빙(SPA fallback), HTTPS 모드 루프백 바인딩
  index.ts        Hono 앱 조립: Env 타입, /api/health, /api/login, 인증 미들웨어, 라우트 마운트. 프론트가 의존하는 유일한 계약면
  auth.ts         scrypt 비밀번호 해시, 버전이 있는 HMAC-SHA256 세션, 인증 미들웨어
...
migrations/       순차 DB 변경(현재 0029까지): 기본 자료/노트/문제/시험, 작업 재시도·멱등성, AI 작업 설정, 지속 AI 작업, 단일 소유자 계정
...
web/
  ...
  src/App.tsx     인증 게이트(마운트 시 subjects 프로브), 상태 기반 라우팅, 3D 배경·커서·ESC 스택 조립
  src/Scene.tsx   react-three-fiber 3D 조형물 배경(페이지 mood·탭 인덱스 반응, lazy + 에러 바운더리)
  src/pages/      Login, Subjects(편집형 인덱스 리스트), SubjectDetail(탭 셸), ChatPanel, MaterialsSidebar,
                  NotesPanel, SourcePicker(자료 다중 선택 공용), Quiz, Wrong(오답 노트 패널), Exam, AISettingsPanel
...
모든 /api/* 는 쿠키 인증 필요(예외: health, auth status, signup, login, logout). 인증 실패 401 `{error}`. unsafe 요청은 교차 Origin을 거부한다. `STUDYWORK_HTTPS_ONLY=true`에서는 HTTPS 프록시 요청만 허용한다. AI 호출 라우트(⚡)는 usage guard(429) + 실패 시 502.
...
- `users(id=1, username, password_hash, session_version, created_at)` — 단일 소유자와 전체 세션 폐기 버전
...
- 디자인 시스템("OBSIDIAN ARCHIVE", styles.css:1-29): 웜 차콜(#0e0e10) 배경 + 본/아이보리(#EDEAE0) 텍스트 + 단일 악센트 **애시드 라임(#D9FF3F)**. 디스플레이 Bricolage Grotesque(대문자 와이드 트래킹 마이크로 라벨), 본문·헤드 Pretendard. react-three-fiber 3D 조형물 배경(Scene.tsx, 페이지·탭 인덱스에 반응, WebGL 실패 시 에러 바운더리로 조용히 생략), 필름 그레인 + 비네트 오버레이, 커스텀 커서(라임 점+링). 모션: 페이지·탭·패널 공용 "페이드+12px 상승"(contentIn), 라인마스크 텍스트 리빌(Reveal), 버튼 프레스 스케일, `prefers-reduced-motion` 전면 비활성. 카피 톤: 건조하게 수치·사실만.
- 라우팅 없음(상태 기반), 상태 라이브러리 없음. 프론트 의존성은 react/marked/katex/dompurify/three 계열뿐.
...
7. **실제 계정 로그인은 HTTPS 전용** — `STUDYWORK_HTTPS_ONLY=true`와 루프백 Tailscale Serve를 함께 사용한다. 이때 Secure `__Host-sw_token`을 제거하거나 직접 IP HTTP를 다시 열지 않는다.
