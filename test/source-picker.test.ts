...
describe("SourcePicker", () => {
  it("검색 범위와 선택 상태를 알리고 닫을 때 검색 상태를 복구한다", async () => {
    ...
    // WCAG 2.5.3: 접근명 = 보이는 텍스트만(컨텍스트 접두 + 라벨 + 카운트) — "전체 선택됨" 같은
    // 상태 서술은 이름에서 제거, 상태는 네이티브 checked/indeterminate(mixed)로만 전달
    expect(bulk.getAttribute("aria-label")).toBe("참고 자료 검색 결과 전체, 1개 검색됨 · 1개 선택");
