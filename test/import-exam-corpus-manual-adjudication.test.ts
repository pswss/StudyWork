...
function q1Q2ExactRecoveryParent5577054(stateDir: string, number: "1" | "2") {
  const rows = {
    "1": [
      "a3a7660f6e9d5a3cd28092bacf4eb31cbd922f6cc46a0b366bf219b470fc73e7",
      "e97b1e1e33e68281b40fcd43dee855962faf601873198fd12b52d68512368b53",
      "f19b967cdc435fe6940232770298319c27b6d974bf0850aa99a9da688e634f20",
    ],
    "2": [
      "9d993c4155eca32cedccbe4de416562cbb1b3bdef35c8d46098db5dbcafa2118",
      "9b6be45c1ea4daac6dd8999c63ba55c802caf97f938a0fbf907bc2c2a11b4eb7",
      "56fce2f819703f943be07ee72993abb1f8f69d6044c4b3cc3bc4d0a749b39f75",
    ],
  } as const;
  const [problemBasis, classificationBasis, parentHash] = rows[number];
  return exactRecoveryParent(
    stateDir,
    `problem-recoveries/v2-0001-000${number}-${problemBasis}.json`,
    `classification-recoveries/v2-0001-000${number}-${classificationBasis}-7bb7cb863c8c4855.json`,
    parentHash,
  );
}
...
describe("exact allowlisted problem manual adjudication", () => {
  it.skipIf(!existsSync(join(q31Q32LiveState, "problem.pdf")))(
    ...
    expect(PROBLEM_MANUAL_ADJUDICATION_ALLOWLIST).toHaveLength(93);
    ...
    expect(canonicalEvidenceHash(PROBLEM_MANUAL_ADJUDICATION_ALLOWLIST))
      .toBe("1b504ee661c23fc93cd1991d18929c535bd4e7f0c27e4c9c8bffa5f6b15bf69e");
  ...
  it.skipIf(!existsSync(join(q43LiveState5577054, "problem.pdf")))(
    ...
    () => {
      ...
      const revisionSpec = PROBLEM_MANUAL_REVISION_ALLOWLIST.find((candidate) =>
        candidate.allowlistId === "ebsi-5577054-q3-source-manual-revision-v1"
      )!;
      ...
      expect({
        ...
        revisionRowHash: canonicalEvidenceHash(revisionSpec),
        ...
        length: 93,
        allowlistHash: "1b504ee661c23fc93cd1991d18929c535bd4e7f0c27e4c9c8bffa5f6b15bf69e",
        ...
        revisionLength: 31,
        revisionAllowlistHash: "706a971562026d94ae52e3eb57678e4f162a65af16127e547a1818c3579bb7f2",
        revisionRowHash: "80610dd155718b0608e794967cdf8f966ae5da6966b8be13b263c10082ffc0b3",
        ...
        terminalRowHash: "9286b633ee4e5ca91378a0c4a07065bde5478801f2d485e5bbf22879646c82cf",
      });
      ...
      expect(revised.question).toContain(
        "관람객이 크게 증가할 것입니다. 그리고 프로그램도 다양화할 수 있습니다. [E]",
      );
      expect(revised.question).not.toContain(
        "관람객이 크게 증가할 것입니다. [E] 그리고 프로그램도 다양화할 수 있습니다.",
      );
      expect(revised.answer).toBe(pinned.failed.question.answer);
    },
  );

  it.skipIf(!existsSync(join(q43LiveState5577054, "problem.pdf")))(
    "pins the source-exact 5577054 Q1-Q2 shared discussion",
    () => {
      const specs = PROBLEM_MANUAL_ADJUDICATION_ALLOWLIST.slice(-2);
      const parents = (["1", "2"] as const).map((number) =>
        q1Q2ExactRecoveryParent5577054(q43LiveState5577054, number)
      );
      const corrected = specs.map((spec, index) => applyAllowlistedProblemManualCorrection(
        spec.entryId,
        spec.sourceHash,
        parents[index].failed.question,
      ));
      const q3Parent = q3ExactRecoveryParent5577054(q43LiveState5577054);
      const q3Spec = PROBLEM_MANUAL_ADJUDICATION_ALLOWLIST.find((candidate) =>
        candidate.allowlistId === "ebsi-5577054-q3-source-manual-v1"
      )!;
      const q3RevisionSpec = PROBLEM_MANUAL_REVISION_ALLOWLIST.find((candidate) =>
        candidate.allowlistId === "ebsi-5577054-q3-source-manual-revision-v1"
      )!;
      const q3Corrected = applyAllowlistedProblemManualCorrection(
        q3Spec.entryId,
        q3Spec.sourceHash,
        q3Parent.failed.question,
      );
      const q3Revised = applyAllowlistedProblemManualRevision(
        q3Spec.entryId,
        q3RevisionSpec.sourceHash,
        q3Spec.allowlistId,
        q3Corrected,
      );
      const sharedPassage = (question: string) => question.split(
        /\n\n(?:토의 참여자의 발화|㉠과 ㉡|다음은 미술관 측에서)/u,
      )[0];
      ...
      expect({
        ...
        corrected: corrected.map((item) => ({
          item: canonicalEvidenceHash(item),
          question: canonicalEvidenceHash(item.question),
        })),
      }).toEqual({
        length: 93,
        allowlistHash: "1b504ee661c23fc93cd1991d18929c535bd4e7f0c27e4c9c8bffa5f6b15bf69e",
        rows: [{
          id: "ebsi-5577054-q1-source-manual-v1",
          row: "d00076512cc072bdaa9de6f476dd44db299aa4593315d1b3ba6c01b8548eeb5b",
          replacements: "5fea78d7b8bc06a2775a73444d89b5162785671b325273eea223e00442c75448",
        }, {
          id: "ebsi-5577054-q2-source-manual-v1",
          row: "10bfd41b879b3fb05b135d96dc04ce2c2d81d8703208b386caacc2aac5a74268",
          replacements: "5c10005fbd361638f4eaa81c174ddfb34014c03bcc44b86d5468b51f16934cd8",
        }],
        corrected: [{
          item: "45ac133ff81a4c284ebafd3649c535ffc0bb50623a4ea526e177fcc5ab029d7a",
          question: "cfddd89209405c613f5656bfcf151b7688caa191f0f554dae3cacf49b8c720d1",
        }, {
          item: "ce6a6b08d815630af5bc200ab01352740a5ab2b2e408c91250355d3bf4f037d7",
          question: "0382d1e38a88ed9e3246544fe7639c515d20f369951873df6840ee17ec61544f",
        }],
      });
      expect(sharedPassage(corrected[0].question)).toBe(sharedPassage(q3Revised.question));
      expect(sharedPassage(corrected[1].question)).toBe(sharedPassage(q3Revised.question));
      expect(corrected.every((item) => !item.question.includes("그만큼 찾는 시민들"))).toBe(true);
      expect(corrected.every((item) => !item.question.includes("전시도 제대로 못합니다"))).toBe(true);
    },
  );
  ...
  it.skipIf(!existsSync(join(q43LiveState5577054, "problem.pdf")))(
    ...
    () => {
      ...
      expect({
        ...
        length: 31,
        allowlistHash: "706a971562026d94ae52e3eb57678e4f162a65af16127e547a1818c3579bb7f2",
      ...
      const q15RevisionSpec = PROBLEM_MANUAL_REVISION_ALLOWLIST.find((candidate) =>
        candidate.allowlistId === "ebsi-5577054-q15-source-manual-revision-v1"
      )!;
      const q15SourceRevisionSpec = PROBLEM_MANUAL_SOURCE_REVISION_ALLOWLIST.find((candidate) =>
        candidate.allowlistId === "ebsi-5577054-q15-source-manual-source-revision-v1"
      )!;
      expect({
        ...
        q15RevisionRow: canonicalEvidenceHash(q15RevisionSpec),
        q15RevisionReplacement: canonicalEvidenceHash(q15RevisionSpec.replacement),
        q15RevisionAdditional: canonicalEvidenceHash(q15RevisionSpec.additionalReplacements),
        q15SourceRevisionRow: canonicalEvidenceHash(q15SourceRevisionSpec),
        q15SourceRevisionReplacement: canonicalEvidenceHash(q15SourceRevisionSpec.replacement),
        ...
        q15RevisionRow: "4687d37a28c5cdcba65dd3b379dac4e9c6d581a8e4821fd3ad7ffa3aeb4498fe",
        q15RevisionReplacement: "c60c27b24eebc3e8129527cee858e0285bc3888327e6935f5092af1e1d993ff8",
        q15RevisionAdditional: "0800080fe6e599be6c3b18f2c3b4493f8dffc69d17e978f9a9d6f277e02be204",
        q15SourceRevisionRow: "1bf7d77120713a5ab54375bbe8cfba16af2c96a983338946d0a4a9b1877117d1",
        q15SourceRevisionReplacement: "254bc40b97c2f2f52d6774ff729b8e28a52de8d62faf4ea3ba0e3d063c77e428",
        sourceLength: 21,
        sourceHash: "835e81e03c1b14b9833d5ab380774c3e7c57f7c96f09aaed28d3e4b81e8a5fe4",
      ...
      expect(PROBLEM_TERMINAL_FIDELITY_ADJUDICATION_ALLOWLIST.slice(-9, -1).map((candidate) => ({
        ...
        { id: "ebsi-5577054-q3-terminal-fidelity-v1", row: "9f48baab546ff4b57dcbec522bab3cf9fcb8831c1a4804f6ea1490bf6714efe6" },
        { id: "ebsi-5577054-q14-terminal-fidelity-v1", row: "fc47f7efd765fd9ce4c8832eb67abe4aeb8e6ce8aa6d0abf276f2898192d6ef0" },
        { id: "ebsi-5577054-q37-terminal-fidelity-v1", row: "5d4fcaae1396f1ce0862b6e107e5015308a9a6c1ff0c8bde1cba2c1f3938968c" },
        { id: "ebsi-5577054-q38-terminal-fidelity-v1", row: "3ab656db717f840a5e694305bd587bba7808ee0c250da6980a723bac2bb61c01" },
        { id: "ebsi-5577054-q39-terminal-fidelity-v1", row: "7f8151f1ca9540af05665ce21f53b6a7cf8fa627b98ac1ed61e416ed315f94c7" },
        { id: "ebsi-5577054-q40-terminal-fidelity-v1", row: "de861f9f6d6493ec175d0f300a2d7b567ad629e13158850f7c0bc819b06881f1" },
      ...
      expect({
        ...
        row: "9286b633ee4e5ca91378a0c4a07065bde5478801f2d485e5bbf22879646c82cf",
      });
      ...
      const q15Manual = applyAllowlistedProblemManualCorrection(q15Spec.entryId, q15Spec.sourceHash, q15Base);
      const q15Revision = applyAllowlistedProblemManualRevision(
        q15Spec.entryId,
        q15RevisionSpec.sourceHash,
        q15Spec.allowlistId,
        q15Manual,
      );
      const q15 = applyAllowlistedProblemManualSourceRevision(
        q15Spec.entryId,
        q15SourceRevisionSpec.sourceHash,
        q15RevisionSpec.allowlistId,
        q15Revision,
      );
      ...
      expect(canonicalEvidenceHash(q15))
        .toBe("27a7d1d54ba68e0656cd92299bda6a4ecc15452103d30e7f76eb1b0e63d98ef4");
      expect(canonicalEvidenceHash(q15.question))
        .toBe("3bfe16f0e61209603f738d91e71562df6abe54bb32989c7a9e3c6a74d20d2476");
      expect(canonicalEvidenceHash(q15.choices))
        .toBe("74e684cf6b73a1f7183c3fe0dff60c8526790d5f2fb8494915374a6df8e90587");
      expect(q15.question).toContain("겨레 소옴 둔 오 닙디 아니 고");
      expect(q15.question).toContain("타락과 초와 쟝과 소금과 계 와");
      expect(q15.choices).toContain(
        "⑤ 둘째 음절의 ‘ㆍ’가 ‘ㅡ’로 변하였음을 (가)의 ‘’과 (나)의 ‘믈’을 통해 확인할 수 있군."
      );
  ...
  it.skipIf(!existsSync(join(q43LiveState5577054, "problem.pdf")))(
    ...
    () => {
      ...
      expect({
        ...
        length: 20,
        allowlistHash: "835e81e03c1b14b9833d5ab380774c3e7c57f7c96f09aaed28d3e4b81e8a5fe4",
  ...
  it.skipIf(!existsSync(join(q43LiveState5577054, "problem.pdf")))(
    ...
    () => {
      ...
      expect({
        ...
        sourceRevisionHash: "835e81e03c1b14b9833d5ab380774c3e7c57f7c96f09aaed28d3e4b81e8a5fe4",
  ...
  it.skipIf(!existsSync(join(q43LiveState5577054, "problem.pdf")))(
    ...
    () => {
      ...
      expect({
        ...
        length: 38,
        allowlistHash: "27f85925154179fa837fd8c7670682d4ea0958877fe3fb1eab4a08ce182f838a",
  ...
  it.skipIf(!existsSync(join(q43LiveState5577054, "problem.pdf")))(
    ...
    () => {
      ...
      expect(PROBLEM_MANUAL_REVISION_ALLOWLIST).toHaveLength(31);
      ...
      expect(PROBLEM_MANUAL_SOURCE_REVISION_ALLOWLIST).toHaveLength(21);
      expect(canonicalEvidenceHash(PROBLEM_MANUAL_SOURCE_REVISION_ALLOWLIST))
        .toBe("835e81e03c1b14b9833d5ab380774c3e7c57f7c96f09aaed28d3e4b81e8a5fe4");
  ...
  it.skipIf(!existsSync(join(q31Q32LiveState, "problem.pdf")))(
    ...
    expect(canonicalEvidenceHash(PROBLEM_TERMINAL_FIDELITY_ADJUDICATION_ALLOWLIST))
      .toBe("27f85925154179fa837fd8c7670682d4ea0958877fe3fb1eab4a08ce182f838a");
  ...
  it.skipIf(!existsSync(q30ManualProblemPath) || !existsSync(q18ManualProblemPath) ||
    ...
    () => {
    expect(PROBLEM_MANUAL_REVISION_ALLOWLIST.slice(0, 17)).toEqual([expect.objectContaining({
    ...
    expect(PROBLEM_MANUAL_REVISION_ALLOWLIST.slice(0, 17).map(canonicalEvidenceHash)).toEqual([
    ...
    const legacySourceRevisionIds = [
      "ebsi-5525982-q32-manual-source-revision-v1",
      "ebsi-5578421-q30-manual-source-revision-v1",
      "ebsi-5578421-q2-manual-source-revision-v1",
      "ebsi-5578421-q21-manual-source-revision-v1",
      "ebsi-5578421-q45-manual-source-revision-v1",
      "ebsi-5577054-q42-manual-source-revision-v1",
      "ebsi-5577054-q25-manual-source-revision-v1",
      "ebsi-5577054-q26-manual-source-revision-v1",
      "ebsi-5577054-q24-source-manual-source-revision-v1",
    ];
    const legacySourceRevisions = PROBLEM_MANUAL_SOURCE_REVISION_ALLOWLIST.filter((spec) =>
      legacySourceRevisionIds.includes(spec.allowlistId)
    );
    expect(legacySourceRevisions).toEqual([expect.objectContaining({
    ...
    expect(legacySourceRevisions.map(canonicalEvidenceHash)).toEqual([
