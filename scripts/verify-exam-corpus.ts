...
function solutionFalseNegativeRepairItemSpec(
  ...
  if (!checkpointSpec || checkpoint.sha256 !== checkpointSpec.sha256
    ...
    || sha256(decision.evidence) !== spec.failedEvidenceHash) {
...
function verifySolutionFalseNegativeRepairAuthority(
  ...
  for (const checkpoint of allowlist.checkpoints) {
    ...
    if (!actual || actual.sha256 !== checkpoint.sha256 || actual.inputHash !== checkpoint.inputHash
      || actual.from !== checkpoint.from || actual.to !== checkpoint.to
      || actual.ownedFrom !== checkpoint.ownedFrom || actual.ownedTo !== checkpoint.ownedTo
      || checkpoint.promptDigest !== SOLUTION_FIDELITY_PROMPT_DIGEST) {
      throw new Error(`${checkpoint.path}: solution false-negative checkpoint authority is stale`);
    }
  }
